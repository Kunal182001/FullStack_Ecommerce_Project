...
const Allproductviewpage = () => {
    ...
    useEffect(() => {
        ...
        fetchData(`/api/products?product=${webId.id}`).then(res => {
            setshowData(res.filterProduct.sort(() => 0.5 - Math.random()))
        });
        if (webId.id == 'Popular' || webId.id == 'New' || webId.id == 'Featured') {
            fetchData('/api/Category?all=true').then(res => {
                setfilterdata(res.Categoreylist);
            })
        }
        else {
            fetchData(`/api/Category?cat=${webId.id}`).then(res => {
                setfilterdata(res.filtercat[0].subcat);
            })
        }
    ...
    useEffect(() => {
        if (selectedFilter != null) {
            ...
            fetchData(`/api/products?product=${selectedFilter}&min=${filterbyprice[0]}&max=${filterbyprice[1]}&rat=${filterbyrating}`).then(res => {
                setshowData(res.filterProduct.sort(() => 0.5 - Math.random()))
            });
        }
    ...
    return (
        <div className='w-full relative flex justify-center items-center  z-1 md:mt-20 mt-[160px]'>
            <div className='w-[90%] flex justify-between items-start'>
                {/* Sidebar filter */}
                <div className='w-[30%] md:w-[20%] hidden md:flex'>
            ...
            {mobfilter &&
                <div className='w-full h-screen z-[105] fixed top-0 bg-white flex flex-col items-start mb-10'>
                    <Sidebar filterdata={filterdata}
                        onFilterChange={handleFilterChange}
                        selectedFilter={selectedFilter}
                        handleFilterPriceChange={handleFilterPriceChange}
                    ...
                    <button onClick={() => { setmobfitler(false) }}
                        className='text-xl mt-5 ml-5 font-bold text-blue-800 flex justify-center gap-2 items-center'>
                        <p>Close</p>
                    </button>
                </div>}
        </div>
...
export default Allproductviewpage
